...
import { CONVERSION_TIER, CONVERSION_MODE, ERROR_MESSAGES, API } from '@config/constants';
import { generateFilename, downloadBlob, downloadFile, showNotification } from '@utils/helpers';
...
class MainConverter {
  ...
  async _convertWithTieredStrategy(paperInfo, mode, mineruToken, onProgress, tabId) {
    ...
    try {
      ...
      showNotification('✅ 转换完成', `已保存：${filename}\n方式：ar5iv (快速模式)`, 'basic');
      ...
      if (mode === CONVERSION_MODE.QUALITY && mineruToken) {
        try {
          ...
          showNotification('⚠️ ar5iv 转换失败', '正在使用 MinerU 深度解析...', 'basic');
  ...
  async _convertWithMinerU(paperInfo, onProgress, tabId) {
    ...
    try {
      ...
      showNotification('✅ 高质量转换完成', `已保存：${filename}\n方式：MinerU (深度解析)`, 'basic');
  ...
  async _fallbackToPdf(paperInfo, onProgress) {
    ...
    try {
      ...
      showNotification('ℹ️ 已保存为 PDF', `文件：${filename}\n建议配置 MinerU Token 以获得 Markdown 转换`, 'basic');
      ...
      showNotification('❌ 转换失败', error.message || ERROR_MESSAGES.UNKNOWN_ERROR, 'basic');
      return { success: false, tier: CONVERSION_TIER.PDF_FALLBACK, error: error.message };
    }
  }
