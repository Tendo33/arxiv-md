...
class MinerUClient {
    ...
    while (attempts < this.maxPollAttempts) {
      ...
      try {
        ...
        if (onProgress && typeof onProgress === 'function') {
          onProgress({ progress: status.progress, state: status.state, attempt: attempts });
        }
  ...
  async convert(pdfUrl, token, metadata = {}, onProgress = null) {
    ...
    try {
      ...
      const markdownWithMeta = this._addMetadata(markdown, { ...metadata, source: 'mineru', taskId });
      if (onProgress) onProgress({ stage: 'completed', progress: 100 });
      logger.info('MinerU conversion successful');
      
      return {
        markdown: markdownWithMeta,
        metadata: { ...metadata, source: 'mineru', taskId, conversionTime: new Date().toISOString() }
      };
    } catch (error) {
